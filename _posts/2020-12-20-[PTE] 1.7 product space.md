...
$$
\begin{align}
\int_{\Omega} f d\mu &= \int_{\Omega_2} \int_{\Omega_1} f(x,y) d\mu_1(x) d\mu_2(y)\\
...
2. If $(X,Y)$ is a random element on a metric space $S \times S$, then $X, Y$ are random elements on $S$. However the converse is not always the truth. In fact, it is known that the converse holds if $S$ is [separable](https://en.wikipedia.org/wiki/Separable_space).[^1] Since $(\mathbb{R}, d)$ is a separable space, we can be sure that $(X,Y)$ is a random vector if and only if $X, Y$ are random variables.
