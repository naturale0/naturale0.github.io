...
We already get a glimpse of stopping times while studying [coupon collector's problem](https://naturale0.github.io/probability/PTE-2.2-Weak-laws-of-large-numbers#weak-law-of-large-numbers) and [renewal theory](https://naturale0.github.io/probability/PTE-2.4-strong-law-of-large-numbers#2-renewal-theory). They were random variables that specify the time that an event occurs. Here, we define it formally.
...
The next example shows that even if a martingale converges almost surely, we cannot guarantee $L^p$ convergence to the same limit. The following sections will be about the condition that a martingale converges a.s. <i>and</i> in $L^p$.
