...
Suppose there is a twin with same physical strength. One of them runs as slowly as he/she can, and the other walks as fast as one can. Who would be the one that enters the end line first?
...
It is our objective to minimize the risk by controlling $\delta$ in various speed $\pi$. Note that if one choose to $\delta_0 = \text{run}$, with a very low speed of $\pi_0 \approx 0$, then one might be unstable and as a result $r(\pi_0, \delta_0)$ rises, and similar happens when one walks with high speed. (Yeah I know it is not so well defined context neither, but let's just skip the details)

Now let's think about this condition in decision theoretic framework. Suppose the environment decides the speed of the race($\pi$) and the runner decides whether to walk or run ($\delta$). The runner's goal here is to minimize the risk(time), while the environment's is to maximize it.

<br>

### 1. If the environment "knows" our choise $\delta$ prior to the runner's decision.

Then it will choose the worst possible choice to us. That is, 

$$ \pi_{\delta} = \text{argmax}_\pi r(\pi, \delta) $$

will be its choice.

Then the runner will try to minimize this by choosing

$$ \delta^* = \text{argmin}_\delta \sup_\pi r(\pi, \delta)$$

which yields to the **minimax** risk:
...
Then he/she will choose the best possible choice to us. That is, 
...
which yields to the **maximin** risk:
...
In this example, we can think of the time of race from the fastest possible walk as $\inf_{\delta} \sup_{\pi} r(\pi, \delta)$, and the time of race from the slowest possible run as $\sup_{\pi} \inf_{\delta} r(\pi, \delta)$.
