...
A probability space is a special kind of a measure space, which is equipped with a positive finite measure. A measure space is defined as a triplet: a space, a $\sigma$-field attached to that space, and a measure function. We will define each of the components.
...
We nect define a function that *measures* sizes of sets inside the $\sigma$-field.
...
The second condition is sometimes referred to as *$\sigma$ sub-additivity*. This is natural if we think of a common notion of a measure: if it is empty, its "size" should be zero and if we add one with another, the resulting size should be the sum. We call $\mu(A)$ the measure of a set $A$.
...
$(\Omega, \mathcal{F})$: a pair of a set and its $\sigma$-field, is called a <b>measurable space</b>. $(\Omega, \mathcal{F}, P)$: a measureable space equipped with a measure is a <b>measurable space</b>. If $P$ is a probability measure, we call this a <b>probability space</b>, $\Omega$ a sample space, and an element of $\mathcal{F}$ an event.
</div>
...
<br>
$\begin{align}
\mathcal{B}(\Omega) &= \text{(the smallest $\sigma$-field that contains all open sets of $\Omega$)} \\
 &= \cap_{\tau\subset\mathcal{F}} \mathcal{F}
\end{align}$
<br>
is the Borel $\sigma$-field of $\Omega$, where $\tau$ is a topology on $\Omega$. An element of $\mathcal{B}(\Omega)$ is called a Borel set.
</div>

An important property of $\sigma$-fields related to the definition of Borel fields is that arbitrary intersections of $\sigma$-fields is a $\sigma$-algebra. (This comes directly from the definition.)
...
An algebra can be generated using semi-algebra. $\overline{\mathcal{S}} := \{\text{finite disjoint unions of sets in }\mathcal{S}\}$ is an **algebra generated by a semi-algebra $\mathcal{S}$**. Like Borel $\sigma$-field, it is the smallest algebra that contains $\mathcal{S}$.
...
The following theorem states that a semi-measure can uniquely extended to a measure on algebra. If the extended measure on algebra is $\sigma$-finite, it can further extended to a measure on $\sigma$-field.
...
Since a distribution fundtion $F$ is a special case of Stieltjes measure function, it follows directly from the theorem that $F$ uniquely determines a probability measure.

On $\mathbb{R}^d,~ d>1$, we define functions similar to Stieltjes measure function: $F:\mathbb{R}^d \to \mathbb{R}^+ \cup \{0\}$ such that $F$ is non-decreasing, right-continuous and $\Delta_A F \ge 0,~ \forall A = (a_1,b_1] \times \cdots \times (a_d,b_d]$, where $\Delta_A F = \sum_{v\in V}\text{sgn}(v)F(v)$, $V=\{a_1, b_1\} \times \cdots \times \{a_d, b_d\}$. Similar to the above, such $F$ can be uniquely extended to a measure $\mu$ such that $\mu(A) = \Delta_A F$ forall finite rectable $A$. <br><br>
